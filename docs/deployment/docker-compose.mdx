...
:::note Prefer the installer scripts for defaults
If you want a guided default setup, use `install.sh` (Linux/macOS) or `install.ps1` (Windows). This page is best when you need advanced Docker Compose customization and production tuning.
:::
...
See [Environment Variables Reference](./environment-variables.mdx) for complete list.
...
Follow migration guide: [Administration → Upgrades](../administration/upgrades.mdx)
...
See [Deployment → Troubleshooting](./troubleshooting.mdx) for more issues.
...
See [Administration → Monitoring](../administration/monitoring.mdx) for:
...
See [Deployment → Backup & Restore](./backup-restore.mdx) for automated backup.
...
See [Administration → Security](../administration/security.mdx).
...
- [Bare-Metal Installation](./bare-metal.mdx)
- [Kubernetes Deployment](./kubernetes.mdx)
- [Deployment Troubleshooting](./troubleshooting.mdx)
