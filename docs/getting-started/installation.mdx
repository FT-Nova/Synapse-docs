...
SYNAPSE is running. Continue to [Your First Agent](./first-agent.mdx) to create your first agent.
...
For production, use [Nginx as a reverse proxy](../deployment/reverse-proxy.mdx).
...
See [Deployment → Kubernetes](../deployment/kubernetes.mdx) for the full guide, including Helm chart usage and resource requirements.
...
- [ ] Set up reverse proxy with TLS for production (see [Reverse Proxy](../deployment/reverse-proxy.mdx))
- [ ] Configured automated backups (see [Backup & Restore](../deployment/backup-restore.mdx))
- [ ] Set up monitoring (see [Monitoring](../administration/monitoring.mdx))

---

## Troubleshooting

Common installation issues: [Deployment → Troubleshooting](../deployment/troubleshooting.mdx)
